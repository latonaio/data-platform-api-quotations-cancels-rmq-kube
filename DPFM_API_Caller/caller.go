...
func (c *DPFMAPICaller) headerCancel(
	...
	header.IsCancelled = input.DeliveryDocument.IsCancelled
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "DeliveryDocumentHeader", "runtime_session_id": sessionID})
	if err != nil {
	...
	for i := range *items {
		(*items)[i].IsCancelled = input.DeliveryDocument.IsCancelled
		res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": (*items)[i], "function": "DeliveryDocumentItem", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			log.Error("%+v", err)
			return nil, nil
		...
		if !checkResult(res) {
			output.SQLUpdateResult = getBoolPtr(false)
			output.SQLUpdateError = "DeliveryDocument Item Data cannot cancel"
			return nil, nil
		}
	}
...
func (c *DPFMAPICaller) itemCancel(
	...
	for _, v := range input.DeliveryDocument.Item {
		data := dpfm_api_output_formatter.Item{
			DeliveryDocument:     input.DeliveryDocument.DeliveryDocument,
	...
	if !*input.DeliveryDocument.Item[0].IsCancelled {
		header := c.HeaderRead(input, log)
		header.IsCancelled = input.DeliveryDocument.Item[0].IsCancelled
		res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "DeliveryDocumentHeader", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			log.Error("%+v", err)
